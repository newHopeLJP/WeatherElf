...
@interface ViewController ()<UICollectionViewDataSource,TodayWeatherViewDelegate>
...
- (void)viewDidLoad {
    ...
    UIPanGestureRecognizer *panGestureRecognizer=[[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(doPan:)];
    
    [self.view addGestureRecognizer:panGestureRecognizer];
//    for (UIGestureRecognizer *gr in self.scrollView.gestureRecognizers) {
//        [gr requireGestureRecognizerToFail:swipGestureRecognizer];
//        NSLog(@"%@",gr);
//    }

    self.view.layer.contents = (id)[UIImage imageNamed:kDEFAULT_BACKGROUND_GRADIENT].CGImage;
    
//    self.view.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageNamed:kDEFAULT_BACKGROUND_GRADIENT]];
...
- (void)doPan:(UIPanGestureRecognizer *)gr{
...
//        NSString *newCity=[[CityPool shareCityPool] cityAfterCity:self.locationName];
//        if ([newCity isEqualToString:self.locationName]) {
//            return;
//        }
//        self.locationName=newCity;
//        [self loadWeatherData];
...
//        vc.locationName=newCity;
//        UIStoryboardSegue *segue=[UIStoryboardSegue segueWithIdentifier:@"rightSegue" source:self destination:vc performHandler:^{
...
//        [self presentViewController:vc animated:YES completion:nil]; 
       ...
       newCity=[[CityPool shareCityPool] cityAfterCity:self.locationName];
    }
...
//        }
        newCity=[[CityPool shareCityPool] cityBeforeCity:self.locationName];
    }
        if ([newCity isEqualToString:self.locationName]) {
...
//            [UIView animateWithDuration:0.4 animations:^{
                self.view.hidden=NO;
                self.pageControl.currentPage=[[CityPool shareCityPool].cities indexOfObject:newCity];
//            }];
        }];
